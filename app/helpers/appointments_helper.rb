...
Rene::App.helpers do
  ...
  def one_is_empty?(hour, date, medic_name)
    hour == "" || date == "" || medic_name == ""
  end

  def validation_error(hour, date, medic_name)
    case
      when hour == "" then "Error: El campo 'Hora' es requerido."
      when date == "" then "Error: El campo 'Fecha' es requerido."
      when medic_name == "" then "Error: El campo 'Nombre de medico' es requerido."
